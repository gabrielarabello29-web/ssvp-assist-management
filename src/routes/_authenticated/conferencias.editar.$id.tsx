...
export const Route = createFileRoute("/_authenticated/conferencias/editar/$id")({
  component: EditarConferenciaPage,
});

function EditarConferenciaPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => conferenciaService.obter(id),
  });

  const { register, handleSubmit, control, reset, formState: { errors } } = useForm<ConferenciaPayload>({
  ...
  useEffect(() => {
    if (data) {
      reset({
        nome: data.nome,
        conselhoParticularId: data.conselho?.id ?? data.conselhoId ?? "",
      });
    }
  }, [data, reset]);

  const mutation = useMutation({
    mutationFn: (payload: ConferenciaPayload) => conferenciaService.atualizar(id, payload),
    onSuccess: () => {
      toast.success("Conferência atualizada");
      qc.invalidateQueries({ queryKey: ["conferencias"] });
      qc.invalidateQueries({ queryKey: ["conferencia", id] });
